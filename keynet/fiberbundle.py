...
def simulation(img_color, fiber_core_x=16, fiber_core_y=16, clad_factor=1.25, shear=1, h_xtalk=0.05, v_xtalk=0.05, pad_val=6, image_pixel_size=4):

    img_color_noisy_FPA = None
    for k_channel in range(0,3):
        img = img_color[:,:,k_channel]
        ...
        big_mask = np.zeros(img.shape)  #make mask filled with zeros same size as image
        ...
        sensitivity_pixel[0:np.int(rows),0:np.int(cols)] = 30  #set to 30, just testing function here
        ...
        # Calculate FPA noise
        noisy_FPA = camera_noise(fiber_out, s_pixel = sensitivity_pixel)

        if img_color_noisy_FPA is None:
            img_color_noisy_FPA = np.zeros( (noisy_FPA.shape[0], noisy_FPA.shape[1], 3) )
        img_color_noisy_FPA[:,:,k_channel] = noisy_FPA

    img_color_noisy_FPA = np.maximum(img_color_noisy_FPA,0.0)
    img_color_noisy_FPA = ((img_color_noisy_FPA - np.min(img_color_noisy_FPA)) / np.max(img_color_noisy_FPA) - np.min(img_color_noisy_FPA))

    #return img_color_noisy_FPA[0:-2*pad_val,:-2*pad_val,:]
    return img_color_noisy_FPA
